...
import com.crafthub.order_service.dto.ErrorResponse;
...
public class GlobalExceptionHandler {
    ...
    // 2. Помилки валідації полів (@Valid)
    ...
    // 3. Помилки доступу (@PreAuthorize та наш custom check)
    @ExceptionHandler({AccessDeniedException.class, com.crafthub.order_service.exception.AccessDeniedException.class})
    public ResponseEntity<ErrorResponse> handleAccessDenied(Exception ex, HttpServletRequest request) {
        return buildResponse(HttpStatus.FORBIDDEN, "Відмовлено в доступі. " + ex.getMessage(), request.getRequestURI(), null);
    }
